...
<div class="content">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select data-placeholder="Tujuan Program" class="form-control form-control-select2" id="tujuanID" data-fouc name="tujuanID">
                                            <?php foreach ($querySub as $group) : ?>
                                                <option value="<?php echo $group['SUBTHEME_ID'] ?>"><?php echo $group['SUBTHEME_ID'] . ' - ' . $group['SUBTHEME_NAME']; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
    ...
    <div id="modalTambahTujuan" class="modal fade" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php echo form_open('Program/simpanTujuanPromes_ajax', ['class' => 'formSimpan']) ?>
...
<script>
    ...
    function edit(id) {
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url('Program/formEditTujuanProgram') ?>",
            data: {
                goalID: id
            },
            dataType: 'json',
            success: function(response) {
                ...
                $('#modalEditTujuan').on('shown.bs.modal', function(e) {
                    $('#namaKelas').focus();
                });
    ...
    function hapus(subTheme) {
        swal({
                title: "Hapus",
                text: "Yakin menghapus data tujuan program semester ?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('Program/hapusTujuanData_ajax') ?>",
                        data: {
                            subTheme: subTheme
                        },
                        dataType: "json",
