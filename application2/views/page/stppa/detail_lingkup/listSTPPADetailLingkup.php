...
=======
                                                    <option value="<?php echo $group['stppa_id'] ?>"><?php echo $group['jenjang_nm'].' '.$group['thn_ajar_periode'] . ' [ Mulai : ' . $group['thn_ajar_tgl_mulai'] . ' ] - [ Akhir :  ' . $group['thn_ajar_tgl_akhir'] . ' ]' ?></option>
>>>>>>> rexydev
                                                <?php endforeach;
                                            else : ?>
...
=======
                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-9">
                                            <div class="form-group">
                                                <label>STPPA Sub lingkup :</label>
                                                <select data-placeholder="Pilih Lingkup" class="form-control " id="sublingkup_id" name="sublingkup_id">
                                                    <option value="">Pilih STPPA Sub Lingkup</option>
>>>>>>> rexydev
                                                </select>
                                            </div>
                                        </div>
...
=======
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Data Detail STPPA</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <?php echo form_open('STPPA/simpanSTPPADetail_ajax', ['class' => 'formSimpan']) ?>
>>>>>>> rexydev
...
=======
                                <div class="">
                                    ...
                                    <select data-placeholder="Pilih Lingkup" class="form-control stppa_sub_lingkup" id="add_stppa_sub_lingkup" name="add_stppa_sub_lingkup">
                                        <option value="">Pilih STPPA Lingkup</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="">
                                    ...
                                    <div class="float-right">
                                        <button type="button" name="add" id="add_detail" class="btn btn-primary mb-1"><span class="icon-add"></span></button>
                                    </div>
                                    <div class="list-add-stppa-detail">
                                        <input type="text" class="form-control" name="add_stppa_detail[]" placeholder="" required="">
>>>>>>> rexydev
                                    </div>
                                </div>
                            </div>
...
<<<<<<< HEAD
                ...
                "data": {
                    "id": $("#lingkup_id option:selected").val()
=======
                "url": "<?= site_url('STPPA/ambilDataSTPPADetail') ?>",
                "type": "POST",
                "data": {
                    "id": $("#sublingkup_id option:selected").val()
>>>>>>> rexydev
                }
            },
...
=======
    $(document).on('click', '#add_detail', function(event) {
>>>>>>> rexydev
...
=======
        $(".list-add-stppa-detail").append(`
            <div class="form-group">
                <div class="input-group"> 
                    <input type="text" class="form-control" name="add_stppa_detail[]" id="add-detail-`+e+`" maxlength="512" placeholder="" required="">
>>>>>>> rexydev
                    `
                    +(e>=1 ? 
                        `
...
=======
        `).on('keyup', '#add-detail-'+e+'', function(event) {
>>>>>>> rexydev
            event.preventDefault();
...
=======
        ...
        $(".stppa_lingkup").change(function(event) {
            $.ajax({
                url: '<?php echo site_url().'STPPA/getSubLingkupSTPPA' ?>',
                type: 'POST',
                dataType: 'json',
                data: {stppa_id: $(".stppa option:selected").val(),id_lingkup:$(".stppa_lingkup option:selected").val()},
                success:function(data){
                    var opt = data.length < 1 ?  ` <option value="">STPPA SubLingkup kosong</option>` :  ` <option value="">Pilih STPPA Sub Lingkup</option>`;
                    
                      data.forEach((item,index)=>{
                        opt+=`
                            <option value="`+item['ssl_id']+`">`+item['ssl_keterangan']+`</option>
                        `
                        // console.log('sss'+item['sl_keterangan']);
                    });
                    $(".stppa_sub_lingkup").html(opt);
                }
            })
            .done(function() {
                console.log("success");
        ...
        $(".stppa_sub_lingkup").change(function(event) {
            var sel = $(".stppa_sub_lingkup option:selected").val();
            $.ajax({
                url: '<?php echo site_url().'STPPA/getDetailSTPPA' ?>',
                type: 'post',
                dataType: 'json',
                data: {id_sublingkup: sel},
                success:function(data){
                    if (data.length>0) {
        ...
        $("#lingkup_id").change(function(event) {
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: {stppa_id: $("#stppa_id option:selected").val(),id_lingkup:$("#lingkup_id option:selected").val()},
                success:function(data){
                    var opt = data.length < 1 ?  ` <option value="">STPPA SubLingkup kosong</option>` :  ` <option value="">Pilih STPPA Sub Lingkup</option>`;
        ...
        $("#sublingkup_id").change(function(event) {
            var sel = $("#sublingkup_id option:selected").val();
            $.ajax({
                url: '<?php echo site_url().'STPPA/getDetailSTPPA' ?>',
                type: 'post',
                dataType: 'json',
...
=======
            url: "<?php echo base_url('STPPA/formEditDetailSTPPA') ?>",
            data: {
                stppa_id: $("#stppa_id option:selected").val(),
                sublingkup_id: $("#sublingkup_id option:selected").val(),
>>>>>>> rexydev
...
=======
                $('#modalEditSTPPADetail').on('shown.bs.modal', function(e) {
                 
                });
                $('#modalEditSTPPADetail').modal('show');
>>>>>>> rexydev
            }
        })
    }
...
=======
                    text: ` ${jumlahData.length} Detail ,Yakin menghapus data  ?`,
>>>>>>> rexydev
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
...
<<<<<<< HEAD
                        ...
                        data: {
                            sub_id: sub_id
=======
                        url: "<?= base_url('STPPA/hapusSTPPADetail_ajax') ?>",
                        data: {
                            detail_id: sub_id
>>>>>>> rexydev
                        },
