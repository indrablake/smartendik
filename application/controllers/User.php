...
class User extends CI_Controller
{
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($res)) {
                ...
                if (password_verify($password, $cek->user_password)) {
                    ...
                    $msg['data'] = $res;
                    // $profile  =  $this->db->query("select p.*,ju.jns_user_nm as jenis_user from dat_profile p inner join ref_jenis_user ju on p.jns_user_kd = ju.jns_user_kd where p.profile_kd='".$res->profile_kd."'")->row();
                    $array = array(
                        'user' => $res
                    );
    ...
    function ambil_data()
    {
        ...
        if ($modul == "kabupaten") {
            echo $this->UserModel->kabupaten($id);
        } else if ($modul == "kecamatan") {
            echo $this->UserModel->kecamatan($id);
        } else if ($modul == "kelurahan") {
            echo $this->UserModel->kelurahan($id);
        }
    }
    ...
    function provinsi()
    {
        ...
        $this->db->order_by('propinsi_nm', 'ASC');
    ...
    function kabupaten($provId)
    {
        ...
        $kab = $this->db->get_where('ref_dati2', array('propinsi_kd' => $provId));
    ...
    function kecamatan($kabId)
    {
        ...
        $this->db->order_by('name', 'ASC');
        $kec = $this->db->get_where('ref_kecamatan', array('dati2_kd' => $kabId));
