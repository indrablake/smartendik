    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item">SNP Keterangan Poin</span>
        ...
        <div class="header-elements d-none">
            <div class="breadcrumb justify-content-center">
                <a href="<?php echo base_url() ?>SNP/formTambahSNPKetPoin" class="btn btn-default btn-sm">Tambah Data</a>
            </div>
        </div>
    </div>
</div>
                            ...
                            <div class="row">
                                <div class="col-md-11">
                                    <div class="form-group">
                                        <label>SNP Poin:</label>
                                        <select data-placeholder="Pilih SNP ID" class="form-control " id="snpID" name="snpID">
                                            <?php $queryGroup = $this->db->query("SELECT snpp.* from snp_poin as snpp where snpp_status='1'")->result_array();
                                            if (!empty($queryGroup)) :
                                                foreach ($queryGroup as $group) : ?>
                                                    <option value="<?php echo $group['snpp_id'] ?>"><?php echo $group['snpp_ket']; ?></option>
                                                <?php endforeach;
                                            else : ?>
                                                <option value="">SNP Kosong</option>
                                            <?php endif;
                                            ?>
                                        </select>
                                    </div>
...
<script>
    ...
    function load_data(query) {
        $.ajax({
            url: "<?php echo base_url(); ?>SNP/fetchSNPKetPoin",
            method: "POST",
            data: {
                query: query
            },
...
<script>
    function showData() {
        table = $('#my_data').DataTable({
            ...
            "ajax": {
                "url": "<?= site_url('SNP/ambilDataSNPKetPoin') ?>",
                "type": "POST",
                "data": {
                    "snpID": $("#snpID").val()
                }
    ...
    function edit(id, snppID) {
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url('SNP/formEditSNPKetPoin') ?>",
            data: {
                spknID: id,
                snppID: snppID
            },
            ...
            success: function(response) {
                ...
                $('#modalEditSNPKetPoin').on('shown.bs.modal', function(e) {
                    $('#namaKelas').focus();
                });
                $('#modalEditSNPKetPoin').modal('show');
            }
        })
    }
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('SNP/hapusSNPKetPoin_ajax') ?>",
                        data: {
                            spknID: snpID,
                            snppID: snppID,
                        },
